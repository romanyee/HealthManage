...
public class HeartBeatView extends View {
    ...
    private void initAttrs(Context context, AttributeSet attrs){
        //获得样式属性
        TypedArray attributes = context.obtainStyledAttributes(attrs,R.styleable.ClockViewAttrs);
        colorDialLower = attributes.getColor(R.styleable.ClockViewAttrs_color_dial_lower, DEFAULT_COLOR_LOWER);//转盘下游颜色
        colorDialMiddle = attributes.getColor(R.styleable.ClockViewAttrs_color_dial_middle, DEFAULT_COLOR_MIDDLE);//转盘中游颜色
        colorDialHigh = attributes.getColor(R.styleable.ClockViewAttrs_color_dial_high, DEFAULT_COLOR_HIGH);//转盘上游颜色
        textSizeDial = (int) attributes.getDimension(R.styleable.ClockViewAttrs_text_size_dial, sp2px(DEFAULT_TEXT_SIZE_DIAL));//文字大小
        strokeWidthDial = (int) attributes.getDimension(R.styleable.ClockViewAttrs_stroke_width_dial, dp2px(DEFAULT_STROKE_WIDTH));//线条宽度
        radiusDial = (int) attributes.getDimension(R.styleable.ClockViewAttrs_radius_circle_dial, dp2px(DEFAULT_RADIUS_DIAL));//转盘半径周期
        titleDial = attributes.getString(R.styleable.ClockViewAttrs_text_title_dial);//转盘标题
        titleDialSize = (int) attributes.getDimension(R.styleable.ClockViewAttrs_text_title_size, dp2px(DEAFAULT_TITLE_SIZE));//转盘标题大小
        titleDialColor = attributes.getColor(R.styleable.ClockViewAttrs_text_title_color, DEAFAULT_COLOR_TITLE);//转盘标题颜色
        valueTextSize = (int) attributes.getDimension(R.styleable.ClockViewAttrs_text_size_value, dp2px(DEFAULT_VALUE_SIZE));//转盘值
        animPlayTime = attributes.getInt(R.styleable.ClockViewAttrs_animator_play_time, DEFAULT_ANIM_PLAY_TIME);//动画时间
    }
    ...
    private void drawArc(Canvas canvas){
        ...
        canvas.drawArc(mRect, 135, 88, false, arcPaint);
        arcPaint.setColor(colorDialMiddle);//转盘中游颜色
        canvas.drawArc(mRect, 223, 88, false, arcPaint);
        arcPaint.setColor(colorDialHigh);//转盘高游颜色
        canvas.drawArc(mRect, 309, 60, false, arcPaint);
    }
    ...
    private void drawPointerLine(Canvas canvas){
        //画布旋转
        canvas.rotate(135);
        for (int i=0; i<151; i++){     //一共需要绘制101个表针
...
//            }
    ...
    private void drawPointerText(Canvas canvas, int i){
        ...
        canvas.rotate(360 - 135 - 1.45f * i);        //坐标系总旋转角度为360度
    ...
    private void drawTitleDial(Canvas canvas){
        ...
        canvas.drawText(currentValue + "次/分钟", 120, radiusDial * 2/3, titlePaint);
    }

    //画旋转的指针
    private void drawPointer(Canvas canvas){
        int currentDegree = (int) (currentValue * 1.45 + 135);
    ...
    //设置完成程度
    public void setCompleteDegree(int degree){

        ValueAnimator animator = ValueAnimator.ofInt(0, degree);
        animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                currentValue = (Math.round((int) animation.getAnimatedValue() * 150)) / 150;
                invalidate();
            }
        });
...
//            @Override
//            public void onAnimationUpdate(ValueAnimator animation) {
//                currentValue = (Math.round((float) animation.getAnimatedValue() * 150)) / 150;
//                invalidate();
//            }
//        });
//        animator.setInterpolator(new AccelerateDecelerateInterpolator());
